...
@section('content')
  ...
  <div class="row">
    ...
    <!-- [ Graphique Camembert Réclamations ] -->
    <div class="col-md-12 col-xl-5 col-xxl-4">
      <div class="card h-100">
        <div class="card-body d-flex flex-column">
          <div class="d-flex align-items-center justify-content-between mb-3">
            <h5 class="mb-0">Répartition des Réclamations</h5>
          </div>
          <div id="reclamations-pie-chart" class="mt-1 flex-grow-1"></div>
          <div class="mt-3 pt-2 border-top">
            <ul class="list-unstyled mb-0">
              <li class="d-flex align-items-center mb-2">
                <span class="badge bg-light-danger text-danger me-2" style="width: 12px; height: 12px; border-radius: 50%;"></span>
                <span>Soumises</span>
                <span class="ms-auto fw-bold">{{ $reclamationsChart['series'][0] ?? 0 }}</span>
              </li>
              <li class="d-flex align-items-center">
                <span class="badge bg-light-success text-success me-2" style="width: 12px; height: 12px; border-radius: 50%;"></span>
                <span>Clôturées</span>
                <span class="ms-auto fw-bold">{{ $reclamationsChart['series'][1] ?? 0 }}</span>
              </li>
            </ul>
          </div>
        </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      setTimeout(function () {
        ...
        if (typeof ApexCharts !== 'undefined' && reclamationChartData.series.length) {
          const options = {
            chart: {
              type: 'pie',
              height: 320
            },
            labels: reclamationChartData.labels,
            series: reclamationChartData.series,
            colors: ['#f06548', '#28c76f'],
            stroke: {
              colors: ['transparent']
            },
            legend: {
            ...
            dataLabels: {
              formatter: function (val, opts) {
                const total = opts.w.config.series.reduce((acc, value) => acc + value, 0);
                const absolute = opts.w.config.series[opts.seriesIndex];
                return `${absolute} (${val.toFixed(1)}%)`;
              }
            }
          };

          const chart = new ApexCharts(document.querySelector('#reclamations-pie-chart'), options);
          chart.render();
        }
      }, 500);
    });
